...
test("should render Tests (2, 3) matching value and expression using deep equality matcher", () => {
  const component = renderer.create(
    <Match expression={{ foo: 1, bar: 2}}>
      <Test value={null}>1</Test>
      <Test value={{ foo: 1 }}>2</Test>
      <Test value={{ bar: 2 }}>3</Test>
    </Match>
  );
  let tree = component.toJSON();
  expect(tree).toMatchSnapshot();
